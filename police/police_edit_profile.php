<?php
 ...
 $emergency_Phone_Number=$rowValues['E_Phone_Number'];
  ...
  if(isset($_POST['Personal_Information'])){
    ...
    $policeTablequery="UPDATE `police` SET `Address`='$Address',`Nationality`='$Nationality' WHERE `Police_Email_FK`='$sessionEmail'";
    $dbConnect->query($query);
    $dbConnect->query($policeTablequery);
    $dbConnect->commit();
    header('location:police_edit_profile.php');
  }
  ...
  if(isset($_POST['Police_Information'])){
    ...
    $policeTablequery="UPDATE `police` SET `Police_Department`='$Police_Department',`Position`='$Position',`Region_Of_Work`='$Region_Of_Work' WHERE `Police_Email_Fk`='$sessionEmail'";
    $dbConnect->query($query);
    $dbConnect->query($policeTablequery);
    $dbConnect->commit();
  ...
  if(isset($_POST['Upload_Profile_Picture'])){
    //path to store the profile picture to profilepicture server
      $targetPath="police_img/police_profile_picture_server/".basename($_FILES['Profile_Picture']['name']);
      $Profile_Picture=$_FILES['Profile_Picture']['name'];
      $dbConnect->autocommit(false);
      $query="UPDATE `police` SET `Police_Profile_picture`='$Profile_Picture' WHERE `Police_Email_FK`='$sessionEmail'";
      $dbConnect->query($query);
      $dbConnect->commit();
  ...
  if(isset($_POST['Upload_Badge_Image'])){
      ...
      $query="UPDATE `account` SET `Badge_image`='$Badge_Image' WHERE `Email`='$sessionEmail'";
      $dbConnect->query($query);
      $dbConnect->commit();
      header('location:police_edit_profile.php');
...
<head>
    ...
    <link rel="stylesheet" href="police_css/police_edit_profile_css.css">
    ...
    <title>Police Edit Profile Page</title>
</head>
<body>
    <div class="wrapper">
        <!------------------NavigationBar--------------------------------->
        ...
        <div class="sidebar">
            ...
            <li><a href="police_dashboard_home.php" >
                ...
                <li><a href="police_edit_profile.php"class="active">
                  ...
                  <h2>Upload Profile Picture</h2>
                    <form method="POST" action="<?= $_SERVER['PHP_SELF'] ?>" enctype="multipart/form-data">
                      <center>
                          <br>
                          <?php
                            echo"<img id='profile_image_preview' src='police_img/police_profile_picture_server/".$rowValues['Police_Profile_picture']."'/>";
